...
void CS(uint3 DTid:SV_DispatchThreadID)
{
	...
	if ( tsdfNormalmap.w < 0.f ){ 
		g_bufOut0[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut1[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut2[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut3[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut4[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut5[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut6[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		return;
	}
	...
	if (kinectPoint.z < range.x || kinectPoint.z >range.y){
		g_bufOut0[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut1[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut2[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut3[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
	...
	if (tsdfPoint.z < range.x || tsdfPoint.z > range.y){
		g_bufOut0[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut1[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut2[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
		g_bufOut3[DTid.x + DTid.y * cb_u2UAVDim.x] = output;
	...
	g_bufOut0[DTid.x + DTid.y * cb_u2UAVDim.x] = float4(cxc, cxn.x);
	g_bufOut1[DTid.x + DTid.y * cb_u2UAVDim.x] = float4(cxn.yz, cyc.yz);
	g_bufOut2[DTid.x + DTid.y * cb_u2UAVDim.x] = float4(cyn, czc.z);
	g_bufOut3[DTid.x + DTid.y * cb_u2UAVDim.x] = float4(czn, nxn.x);
	g_bufOut4[DTid.x + DTid.y * cb_u2UAVDim.x] = float4(nxn.yz, nyn.yz);
	g_bufOut5[DTid.x + DTid.y * cb_u2UAVDim.x] = float4(nzn.z, cpqn);
	g_bufOut6[DTid.x + DTid.y * cb_u2UAVDim.x] = float4(npqn, 1);
