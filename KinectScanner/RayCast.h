...
public:
	...
	RayCast( float volumeVoxel_size, UINT volumeVoxel_x, UINT volumeVoxel_y, UINT volumeVoxel_z,
			 CModelViewerCamera* pCamera = NULL, bool RTTexture = false, 
	...
	void Resize()
	{
		if( !m_bExternalCam ){
			...
			m_pCamera->SetProjParams( XM_PI / 4, fAspectRatio, 0.01f, 500.0f );
			m_cbPerFrame.vWinTR.x = tan( XM_PI / 8.0f );
			m_cbPerFrame.vWinTR.y = m_cbPerFrame.vWinTR.x / fAspectRatio;

			m_pCamera->SetWindow( m_uRTwidth, m_uRTheight );
			m_pCamera->SetButtonMasks( MOUSE_MIDDLE_BUTTON, MOUSE_WHEEL, MOUSE_LEFT_BUTTON );
		}
	}
	...
	void Release()
	{
		...
		if(!m_bExternalCam) delete m_pCamera;
	}
	...
	void Render( ID3D11DeviceContext* pd3dImmediateContext )
	{
		...
		XMMATRIX m_WorldView = m_World * m_View;
		XMMATRIX m_WorldViewProjection = m_WorldView*m_Proj;
		XMVECTOR t;


		m_cbPerFrame.mWorldViewProjection = XMMatrixTranspose( m_WorldViewProjection );
		m_cbPerFrame.mInvWorldView = XMMatrixTranspose( XMMatrixInverse( &t, m_WorldView ) );
		XMStoreFloat4( &m_cbPerFrame.vViewPos, m_pCamera->GetEyePt() );
