...
Texture3D g_DensityWeightVol : register(t0);
Texture3D g_ColorVol : register(t1);
...
float4 PS(PS_INPUT input) : SV_Target
{
	...
	float field_pre ;
	float field_now = g_DensityWeightVol.SampleLevel(samRaycast,P * inverseXYZsize.xyz + 0.5,0).x;

	float isoValue = inverseXYZsize.w;
	
	while ( t <= tfar ) {
		float3 txCoord = P * inverseXYZsize.xyz + 0.5;
		float4 Field =  g_DensityWeightVol.SampleLevel ( samRaycast, txCoord, 0 );
		...
		if ( field_now > isoValue && field_pre < isoValue )
		{
			...
			float depth_dx = g_DensityWeightVol.SampleLevel ( samRaycast, txCoord, 0, int3 ( 1, 0, 0 ) ).x - 
								g_DensityWeightVol.SampleLevel ( samRaycast, txCoord, 0, int3 ( -1, 0, 0 ) ).x;
			float depth_dy = g_DensityWeightVol.SampleLevel ( samRaycast, txCoord, 0, int3 ( 0, 1, 0 ) ).x - 
								g_DensityWeightVol.SampleLevel ( samRaycast, txCoord, 0, int3 ( 0, -1, 0 ) ).x;
			float depth_dz = g_DensityWeightVol.SampleLevel ( samRaycast, txCoord, 0, int3 ( 0, 0, 1 ) ).x - 
								g_DensityWeightVol.SampleLevel ( samRaycast, txCoord, 0, int3 ( 0, 0, -1 ) ).x;
