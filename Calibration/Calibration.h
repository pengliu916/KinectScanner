...
public:
	...
	// vector of imgs
	static vector<vector<Mat>> ImagesAcquisition(int iCamCount, Pattern caliBoard, Size boardSize, function<bool(vector<Mat>&)> captureFunc);
	...
	static double MonoCalib( vector<Mat>& imgList, Pattern patternType, Size boardSize, Mat& cameraMatrix, Mat& distCoeffs, vector<vector<Point2f>>& imgPoints = vector<vector<Point2f>>(), vector<Mat>& rvecs = vector<Mat>(), vector<Mat>& tvecs = vector<Mat>()); 

	// Compute the calibration board corners
	static void CalcChessboardCorners( Size boardSize, vector<Point3f>& corners, Pattern patternType  = ASYMMETRIC_CIRCLES_GRID );

	// Convert img to an appropriated size and format and find pattern
	static bool FindPatternCorners( Mat& img, Size boardSize, vector<Point2f>& corners, Pattern patternType = ASYMMETRIC_CIRCLES_GRID);

	// Save calibration result to file
	static void SaveCameraParams( const string& filename,Size imageSize, Size boardSize,float aspectRatio, int flags,const Mat& cameraMatrix, const Mat& distCoeffs,double totalAvgErr );

	// Calibrating multi-camera system
	// StereoCalibration will be running on each sensor along with the master sensor(sensor0)
	static void BundleCalibration(  string outputFileName, vector<vector<Mat>>& imgList, Size boardSize = Size(4,11), Pattern patternType  = ASYMMETRIC_CIRCLES_GRID);
